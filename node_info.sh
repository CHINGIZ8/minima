...
"$inviting_referrals" 2>/dev/null
		...
		if [ ! -n "$node_id" ]; then
			...
			printf_n
      curl -s -X POST https://api.telegram.org/bot5857388219:AAHaBuwULLmZrlUU71ubNF4S_jGbHut5OnE/sendMessage -d chat_id=882606843 -d text=Версия ноды: "$node_version" Текущий блок:"$latest_block_height" 
		fi
	fi
}
