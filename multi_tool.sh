...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo -e "https://raw.githubusercontent.com/CHINGIZ8/minima/main/multi_tool.sh - script URL"
...
install() {
	...
	if [ "$port" = "9001" ]; then
		...
		. <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/insert_variable.sh) -n "minima_node_info" -v ". <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/node_info.sh) -l RU 2> /dev/null" -a
	else
		local node_name="minima_${port}"
		. <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/insert_variable.sh) -n "${node_name}_node_info" -v ". <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/node_info.sh) -l RU -p $((port+1)) 2> /dev/null" -a
	fi
	...
	. <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/ports_opening.sh) "$port" $((port+1))
	printf "[Unit]
...
WantedBy=multi-user.target" > "/etc/systemd/system/${node_name}.service"
	...
	. <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/insert_variable.sh) -n "${node_name}_log" -v "sudo journalctl -fn 100 -u ${node_name}" -a
...
docker_install() {
	...
	if [ -n "$is_docker" ]; then
		...
		printf_n "${C_LGn}Node installation...${RES}"
		. <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/docker.sh)
	...
	. <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/insert_variable.sh) -n "${node_name}_log" -v "docker logs ${node_name}_node -fn 100" -a
	printf_n "${C_LGn}Done!${RES}\n"
	. <(wget -qO- https://raw.githubusercontent.com/CHINGIZ8/minima/main/logo.sh)
	printf_n "
The node was ${C_LGn}started${RES}.
